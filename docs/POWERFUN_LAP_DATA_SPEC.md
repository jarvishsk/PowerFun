...
| **区域填充** | 前 N 次最高配速 vs 最低配速之间用颜色填充（注意：配速数值小=快，所以"最高配速"=最慢，"最低配速"=最快） |
| **配色** | 本次配速用主色（如蓝色），心率用红色，历史平均用灰色虚线，填充区域用浅蓝半透明 |

**关键细节**:
- **平滑曲线**：使用 Plotly 的 `line.shape='spline'`，不要用均值聚合
- **前 N 次**：同类型跑步（`easy_run`/`aerobic_run`/`lsd`/`race_event`），N = min(5, 可用的同类型跑步数量)，N ≥ 1
- **最高/最低配速**：对于每个公里序号 K，取前 N 次该公里配速的 max/min，不是取 N 次的均值再算极值
