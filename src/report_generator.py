...
class ReportGenerator:
    ...
    def _render_fallback(self, df, charts_json, stats, insights, table_data, charts,
        ...
        # 构建HTML各部分
        insights_html = '\n'.join([
            f'<div class="insight-card {i["type"]}">'
            f'<div class="icon">{i["icon"]}</div>'
            f'<div class="title">{i["title"]}</div>'
            f'<div class="message">{i["message"]}</div></div>'
            for i in insights
        ])

        table_html = '\n'.join([
            f'<tr><td>{r["date"]}</td><td>{r["title"]}</td>'
            f'<td><span class="category-badge" style="background-color:{r["category_color"]}">{r["category"]}</span></td>'
            f'<td>{r["distance"]}</td><td>{r["pace"]}</td><td>{r["hr"]}</td>'
            f'<td>{r["power"]}</td><td>{r["cadence"]}</td></tr>'
            for r in table_data
        ])
