...
class DataProcessor:
    ...
    def _first_not_none(*values):
        """返回第一个非 None 的值（兼容 0.0 等数值，不能用 or）"""
        for v in values:
            if v is not None:
                return v
        return None
    ...
    def _extract_extra(self, summary: dict, field: str, activity: dict):
        ...
        value = self._first_not_none(activity.get(field), summary.get(field))
        if value is not None:
            return value
        ...
        if field == "training_effect":
            ...
            return self._first_not_none(summary.get("maxPower"), activity.get("maxPower"))
        elif field == "normalized_power":
            return self._first_not_none(
                activity.get("normalizedPower"), summary.get("normalizedPower"),
                activity.get("normPower"))
        elif field == "aerobic_training_effect":
            return self._first_not_none(
                activity.get("aerobicTrainingEffect"), summary.get("aerobicTrainingEffect"))
        elif field == "anaerobic_training_effect":
            return self._first_not_none(
                activity.get("anaerobicTrainingEffect"), summary.get("anaerobicTrainingEffect"))
        elif field == "vO2_max":
            return self._first_not_none(
                activity.get("vO2Max"), summary.get("vO2Max"), activity.get("vO2MaxValue"))
            ...
            return self._first_not_none(
                summary.get("groundContactTime"), activity.get("avgGroundContactTime"),
                activity.get("groundContactTime"))
        elif field == "activity_id":
            return self._first_not_none(
                activity.get("activity_id"), activity.get("activityId"))
        elif field == "location":
            return self._first_not_none(
                activity.get("locationName"),
                activity.get("activity_name", "").replace(" 跑步", ""))
        elif field == "elevation_loss_m":
            return summary.get("elevationLoss")
        elif field == "steps":
            return self._first_not_none(summary.get("steps"), activity.get("steps"))
        elif field == "start_lat":
            return self._first_not_none(
                summary.get("startLatitude"), activity.get("start_latitude"))
        elif field == "start_lon":
            return self._first_not_none(
                summary.get("startLongitude"), activity.get("start_longitude"))
